 ...
 // console.log(localStorage.getItem('token'));
  ...
  const fetchaccount= async ()=>
  {
    ...
    try{
      ...
      const response=await axios.get("http://localhost:8001/farmer/myaccount",
      {
        headers:
        {
  ...
  const handleDeleteClick =async (ProdID) => {
    ...
    console.log(ProdID);
        try {
          const response = await axios.delete('http://localhost:8001/farmer/product/'+ProdID, {     headers:
          {
              Authorization: 'Bearer ' + localStorage.getItem('token')
          } });
          fetchaccount();
