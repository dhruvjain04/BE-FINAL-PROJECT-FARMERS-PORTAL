...
const Register = () => {
    ...
    const onClickRegister = async (event) => {
        ...
        if (user.role === 'farmer') {
            url = 'http://localhost:8001/farmer/farmerSignup'; // Replace with the farmer signup route
          } else if (user.role === 'exporter') {
            url = 'http://localhost:8001/user/exporterSignup'; // Replace with the exporter signup route
          }
